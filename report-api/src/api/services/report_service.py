...
class ReportService:
    ...
    def create_report_from_stored_template(cls, template_name: str, template_args: object):
        ...
        html_template = template.render(template_args, bclogoUrl=bc_logo_url, registriesurl=registries_url)
        html_out = HTML(string=html_template, base_url=request.base_url)
        print(html_out)
        pdf_out = html_out.write_pdf(presentational_hints=True)
        return pdf_out
