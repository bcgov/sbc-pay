...
def FRONTEND_DIRS = ['pay_ui']
def FRONTEND_FILE_TYPES = ['html', 'vue', 'ts', 'js']
def BACKEND_DIRS = ['pay-api','report-api']
def BACKEND_FILE_TYPES = ['py']

def SONAR_ROUTE_NAME = 'sonarqube'
def SONAR_ROUTE_NAMESPACE = 'l4ygcl-tools'
def SONAR_PROJECT_NAME = 'BC Registries Payment'
def SONAR_PROJECT_KEY = 'BCRegistriesPay'
def SONAR_PROJECT_BASE_DIR = '../'
def SONAR_SOURCES = './'

def SONAR_PROJECT_BACKEND_KEY = 'BCRegistriesPayAPI'
...
if( run_pipeline ) {
  // create pay pod on openshift to run verification steps
  def pod_label = "pay-pod-${UUID.randomUUID().toString()}"
  ...
  podTemplate(
    ...
    containers: [
      containerTemplate(
        name: 'jnlp',
        image: 'docker-registry.default.svc:5000/l4ygcl-tools/jenkins-slave-python3-nodejs10:latest',
        ...
        envVars: [
            ...
            secretEnvVar(key: 'JWT_OIDC_ALGORITHMS', secretName: 'apitest-secrets', secretKey: 'JWT_OIDC_ALGORITHMS'),
            secretEnvVar(key: 'JWT_OIDC_AUDIENCE', secretName: 'apitest-secrets', secretKey: 'JWT_OIDC_AUDIENCE'),
            secretEnvVar(key: 'JWT_OIDC_CLIENT_SECRET', secretName: 'apitest-secrets', secretKey: 'JWT_OIDC_CLIENT_SECRET'),
            secretEnvVar(key: 'JWT_OIDC_ISSUER', secretName: 'apitest-secrets', secretKey: 'JWT_OIDC_ISSUER'),
            secretEnvVar(key: 'JWT_OIDC_JWKS_CACHE_TIMEOUT', secretName: 'apitest-secrets', secretKey: 'JWT_OIDC_JWKS_CACHE_TIMEOUT'),
            secretEnvVar(key: 'JWT_OIDC_WELL_KNOWN_CONFIG', secretName: 'apitest-secrets', secretKey: 'JWT_OIDC_WELL_KNOWN_CONFIG'),
            secretEnvVar(key: 'PAYBC_BASE_URL', secretName: 'apitest-secrets', secretKey: 'PAYBC_BASE_URL'),
            secretEnvVar(key: 'PAYBC_CLIENT_ID', secretName: 'apitest-secrets', secretKey: 'PAYBC_CLIENT_ID'),
            secretEnvVar(key: 'PAYBC_CLIENT_SECRET', secretName: 'apitest-secrets', secretKey: 'PAYBC_CLIENT_SECRET'),
            secretEnvVar(key: 'PAYBC_PORTAL_URL', secretName: 'apitest-secrets', secretKey: 'PAYBC_PORTAL_URL'),
            secretEnvVar(key: 'REPORT_API_BASE_URL', secretName: 'apitest-secrets', secretKey: 'REPORT_API_BASE_URL')
        ]
      )
    ]
  ){
    node(pod_label) {
      ...
      BACKEND_DIRS.each {
        def apiApp = "${it}"
        def apiAppSrc = "src/"+ "${it}"

        def sonarProjectName = "${SONAR_PROJECT_NAME} - ${apiApp} - ${env.BRANCH_NAME}"
        def sonarProjectKey = "${SONAR_PROJECT_BACKEND_KEY}-${apiApp}-${env.BRANCH_NAME}"
          ...
          stage('flake8') {
            ...
            try{
              sh '''
                ...
                source /opt/app-root/bin/activate
                export PYTHONPATH=./src/
                flake8 ${apiAppSrc} tests --exit-zero > flake8.txt
              '''
            } catch (Exception e) {
              echo "EXCEPTION: ${e}"
            } finally {
              def flake8 = scanForIssues tool: flake8(pattern: 'flake8.txt')
              publishIssues id: 'flake8-'+ "${apiApp}", ignoreQualityGate: true, issues: [flake8], name: 'flake8-'+ "${apiApp}"

              pullrequestComment("${GITHUB_TOKEN}", "${GITHUB_API_URL}" + "/issues/${env.CHANGE_ID}/comments", "Flake8 : " + "${env.BUILD_URL}" + "flake8-${apiApp}/")
            }
          }
          ...
          stage('pylint') {
            ...
            try{
              sh '''
                ...
                source /opt/app-root/bin/activate
                export PYTHONPATH=./src/
                pylint --rcfile=setup.cfg --load-plugins=pylint_flask --disable=C0301,W0511 ${apiAppSrc} --exit-zero --output-format=parseable > pylint-report.txt
              '''
            } catch (Exception e) {
              echo "EXCEPTION: ${e}"
            } finally {
              ...
              publishIssues id: 'pyLint-'+ "${apiApp}", ignoreQualityGate: true, issues: [pyLint], name: 'pyLint-'+ "${apiApp}"
            }
          }
        ...
        stage("Quality Gate - API") {
          timeout(time: 2, unit: 'MINUTES') {
            ...
            pullrequestStatus("${env.GITHUB_TOKEN}",
                              ...
                              'continuous-integration/QualityCheck-' + "${apiApp}",
                              stage_description,
