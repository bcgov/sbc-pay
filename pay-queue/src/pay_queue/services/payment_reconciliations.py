...
from pay_api.models import AppliedCredits, db
            ...
            case RecordType.EFTP.value:
                ...
                if payment is None and current_app.config.get("SKIP_EXCEPTION_FOR_TEST_ENVIRONMENT"):
                    current_app.logger.warning(
                        "Payment not found for receipt number %s skipping creation",
                        source_txn_number,
                    )
                    continue
...
def _process_file_content(
    ...
    try:
        ...
        error_msg = f"Error creating payment records: {e!s}"
        has_errors = True
    ...
    try:
        ...
        error_msg = f"Error creating credit records: {e!s}"
        has_errors = True
        _csv_error_handling("N/A", error_msg, error_messages, e)
        return has_errors, error_messages
    ...
    try:
        ...
        error_msg = f"Error syncing credit records: {e!s}"
        has_errors = True
        _csv_error_handling("N/A", error_msg, error_messages, e)
        return has_errors, error_messages
...
def _handle_applied_credit(row):
    ...
    if AppliedCredits.find_by_application_id(application_id):
        current_app.logger.warning(f"Applied Credit exists with application_id {application_id}.")
        return
    ...
    applied_credits = Decimal(_get_row_value(row, Column.APP_AMOUNT))
    # Iterate over invoice reference highest number to lowest number, that way credit is shown on the earliest pages
    # For online banking this will only ever be one invoice.
    invoices = (
    ...
    for invoice in invoices:
        ...
        # Invoice paid is already updated in apply credit endpoint
        # Invoice status gets updated in _process_paid_invoices
    ...
    for credit in active_credits:
        ...
        if credit.is_credit_memo:
            try:
                ...
                if current_app.config.get("SKIP_EXCEPTION_FOR_TEST_ENVIRONMENT"):
                    current_app.logger.warning(f"Error fetching credit memo {credit.cfs_identifier} : {e!s}")
                    continue
                raise e
        ...
        gcp_queue_publisher.publish_to_queue(
            QueueMessage(
                ...
                attributes={
                    ...
                    "corpTypeCode": inv.corp_type_code,
                    "paymentMethod": inv.payment_method_code,
                },
            ),
        )
...
def _publish_online_banking_mailer_events(rows: list[dict[str, str]], paid_amount: float):
    ...
    payload = {
        ...
        "amount": f"{paid_amount:.2f}",  # pylint: disable = consider-using-f-string
        "creditAmount": f"{credit_amount:.2f}",  # pylint: disable = consider-using-f-string
    }
