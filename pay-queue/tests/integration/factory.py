...
from pay_api.utils.enums import (
    ...
    PaymentSystem,
    RoutingSlipStatus,
    TransactionStatus, RefundStatus, RefundType,
)
...
def factory_create_eft_account(auth_account_id="1234", status=CfsAccountStatus.ACTIVE.value, cfs_account="1234"):
    """Return Factory."""
    account = PaymentAccount(
        auth_account_id=auth_account_id,
        payment_method=PaymentMethod.EFT.value,
        name=f"Test EFT {auth_account_id}",
    ).save()
    CfsAccount(
        status=status,
...
def factory_refund(routing_slip_id: int = None, details={}, invoice_id: int = None, status=RefundStatus.APPROVAL_NOT_REQUIRED.value):
    """Return Factory."""
    return Refund(
        invoice_id=invoice_id,
