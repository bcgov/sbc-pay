#!/bin/bash
# Creates two GCP log-based metrics for pay-api dashboard monitoring,
# using Cloud Run's httpRequest.latency directly.
#
# Uses 5 labels to identify every endpoint without including dynamic values:
#
#   route_group  — first path segment: payment-requests, accounts, eft-shortnames, fees, fas, ...
#   sub_route    — known literal sub-resource, skipping any dynamic segment:
#                  receipts, refunds, statements, distributions, routing-slips, ...
#   has_id       — "/" when an integer ID is present in the URL, null otherwise
#   method       — HTTP method: GET, POST, PUT, PATCH, DELETE
#   api_version  — v1 or v2 (null for /ops/ routes)
#
...
case "$ENV" in
  dev)     PROJECT="gtksf3-dev";   SERVICE="pay-api-dev"     ;;
  test)    PROJECT="gtksf3-test";  SERVICE="pay-api-test"    ;;
  sandbox) PROJECT="gtksf3-tools"; SERVICE="pay-api-sandbox" ;;
  prod)    PROJECT="gtksf3-prod";  SERVICE="pay-api-prod"    ;;
...
# Plural/longer forms must precede their singular prefixes so the alternation
# doesn't short-circuit: "payments" before "payment", "statements" before "statement".
SUB_ROUTE_ALT = (
    "comments|composite|distributions|fees|healthz|history|"
    "info|invoices|links|manual|notifications|nsf|payments|payment|"
    "queries|readyz|receipts|refunds|reports|routing-slips|schedules|"
    "search|settings|shortname-refund|statements|statement|summaries|"
    "summary|transactions|valid_payment_methods|verifications"
)

labels = [
    {"key": "route_group", "valueType": "STRING", "description": "API resource (payment-requests, accounts, eft-shortnames, fees, fas, ...)"},
...
extractors = {
    # First segment after /api/vN/ — captures payment-requests, accounts, eft-shortnames, fas, etc.
    "route_group": 'REGEXP_EXTRACT(httpRequest.requestUrl, "/api/v[12]/([^/?]+)")',
    # Greedy (?:[^/]+/)* skips all dynamic segments; captures the last known literal sub-route.
    # Works for paths up to 4 levels deep (e.g. /accounts/{id}/statements/notifications).
    "sub_route":   f'REGEXP_EXTRACT(httpRequest.requestUrl, "/api/v[12]/[^/]+/(?:[^/]+/)*({SUB_ROUTE_ALT})")',
    # Captures "/" when any integer ID segment appears anywhere in the path.
    "has_id":      'REGEXP_EXTRACT(httpRequest.requestUrl, "(/)[0-9]+")',
...
latency_metric = {
    "name": "pay_api_request_latency",
    "description": "pay-api per-endpoint request latency in seconds",
    "filter": base_filter,
    "metricDescriptor": {
        "metricKind": "DELTA",
...
count_metric = {
    "name": "pay_api_request_count",
    "description": "pay-api per-endpoint request count",
    "filter": base_filter,
    "metricDescriptor": {
        "metricKind": "DELTA",
        "valueType": "INT64",
...
create_or_update "pay_api_request_latency" "$WORK_DIR/latency.json"

echo ""
echo "Creating request count metric..."
create_or_update "pay_api_request_count" "$WORK_DIR/count.json"
...
echo "  route_group  — resource dropdown (payment-requests, accounts, eft-shortnames, fees, fas, ...)"
echo "  sub_route    — sub-resource dropdown (receipts, refunds, statements, distributions, routing-slips, ...)"
