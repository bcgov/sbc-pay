...
class Payment(BaseModel):  # pylint: disable=too-many-instance-attributes
    ...
    def search_purchase_history(cls,  # noqa:E501; pylint:disable=too-many-arguments, too-many-locals, too-many-branches, too-many-statements;
        ...
        if not return_all:
            ...
            # Add pagination
            sub_query = cls.generate_subquery(auth_account_id, search_filter, limit, page=page)
            ...
            sub_query = cls.generate_subquery(auth_account_id, search_filter, limit, page=None)
            result, count = query.filter(Invoice.id.in_(sub_query)).all(), sub_query.count()
        else:
            count = cls.get_count(auth_account_id, search_filter)
            if count > 60000:
