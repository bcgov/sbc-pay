...
"""Service to manage Direct Sale PAYBC Payments.
...
class DirectSaleService(PaymentSystemService, OAuthService):
    ...
    def get_payment_system_url_for_invoice(self, invoice: Invoice, inv_ref: InvoiceReference, return_url: str):  # noqa: ARG002
        ...
        url_params_dict = {
            ...
            "currency": "CAD",
            "revenue": DirectSaleService._create_revenue_string(invoice),
        }
    ...
    def _create_revenue_string(invoice) -> str:
        ...
        for payment_line_item in payment_line_items:
            ...
            if payment_line_item.total > 0:
                index += 1
                revenue_string = DirectSaleService._get_gl_coding(distribution_code, payment_line_item.total)
                revenue_item.append(f"{index}:{revenue_string}")
                index = DirectSaleService._append_revenue_item(
                    index,
    ...
    def _append_revenue_item(index, code_id, amount, revenue_item):
        if amount is not None and amount > 0 and code_id is not None:
            ...
            revenue_string = DirectSaleService._get_gl_coding(code, amount)
            revenue_item.append(f"{index}:{revenue_string}")
        return index
    ...
    def _build_refund_revenue_lines(refund_partial: list[RefundPartialLine]):
        ...
        for refund_line in refund_partial:
            ...
            revenue_account = DirectSaleService._get_gl_coding(
    ...
    def build_automated_refund_payload(cls, invoice: InvoiceModel, refund_partial: list[RefundPartialLine]):
        ...
        refund_lines, total_refund = DirectSaleService._build_refund_revenue_lines(refund_partial)
        paybc_invoice = DirectSaleService.query_order_status(invoice)
        refund_payload.update(
            {
                "refundRevenue": DirectSaleService._build_refund_revenue(paybc_invoice, refund_lines),
