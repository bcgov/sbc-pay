...
class PaymentService:  # pylint: disable=too-few-public-methods
    ...
    def create_invoice(cls, payment_request: Tuple[Dict[str, Any]], authorization: Tuple[Dict[str, Any]]) -> Dict:
        ...
        try:
            ...
            invoice.service_fees = Decimal(str(sum(fee.service_fees for fee in fees))) if fees else 0
            invoice.total = Decimal(str(sum(fee.total for fee in fees))) if fees else 0
            invoice.paid = 0
            invoice.refund = 0
