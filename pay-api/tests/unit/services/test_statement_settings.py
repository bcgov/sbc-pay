...
from pay_api.services.statement_settings import (
    StatementSettings as StatementSettingsService,
)
...
def test_statement_settings_find_by_account(session):
    ...
    factory_statement_settings(payment_account_id=bcol_account.id, frequency=StatementFrequency.DAILY.value)
...
def test_update_statement_daily(session):
    ...
    current_statement_settings = StatementSettingsModel.find_active_settings(
        payment_account.auth_account_id, datetime.now(tz=timezone.utc)
    )
    ...
    end_of_month_date = get_first_and_last_dates_of_month(
        datetime.now(tz=timezone.utc).month, datetime.now(tz=timezone.utc).year
    )[1]
    ...
    statement_settings = StatementSettingsService.update_statement_settings(
        payment_account.auth_account_id, StatementFrequency.DAILY.value
    )
    assert statement_settings is not None
    ...
    assert statement_settings.get("from_date") == (datetime.now(tz=timezone.utc).date() + timedelta(days=1)).strftime(
        DT_SHORT_FORMAT
    )
...
def test_update_statement_daily_to_daily(session):
    ...
    assert statement_settings.get("from_date") == (datetime.now(tz=timezone.utc) + timedelta(days=1)).strftime(
        DT_SHORT_FORMAT
    )
...
def test_update_statement_monthly(session):
    ...
    i.save()
    factory_invoice_reference(i.id).save()
    factory_statement_settings(payment_account_id=bcol_account.id, frequency=StatementFrequency.MONTHLY.value)
...
def test_update_statement_weekly(session):
    ...
    i.save()
    factory_invoice_reference(i.id).save()
    factory_statement_settings(payment_account_id=bcol_account.id, frequency=StatementFrequency.WEEKLY.value)
