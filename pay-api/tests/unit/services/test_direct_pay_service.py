...
"""Tests to assure the DirectPay service layer.

Test-Suite to ensure that the DirectPay layer is working as expected.
"""
...
def test_create_account(session):
    """Test create_account."""
    account = direct_pay_service.create_account(identifier="100", contact_info={}, payment_info={})
    assert account
...
def test_get_payment_system_url_for_invoice(session):
    ...
    payment_system_url = direct_pay_service.get_payment_system_url_for_invoice(invoice, invoice_reference, "hello")
...
@patch("pay_api.services.direct_pay_service.CFSService.get_token")
def test_get_receipt_no_receipt_number(mock_get_token, session):
    ...
    with (
        ...
        result = direct_pay_service.get_receipt(payment_account, pay_response_url, invoice_reference)
