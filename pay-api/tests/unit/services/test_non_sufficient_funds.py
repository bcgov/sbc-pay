...
def test_find_all_non_sufficient_funds_invoices(session):
    ...
    s1_settings = factory_statement_settings(payment_account_id=payment_account.id,
                                             frequency=StatementFrequency.MONTHLY.value).save()
    statement = factory_statement(payment_account_id=payment_account.id,
                                  frequency=StatementFrequency.MONTHLY.value,
                                  statement_settings_id=s1_settings.id).save()
    factory_statement_invoices(statement_id=statement.id,
