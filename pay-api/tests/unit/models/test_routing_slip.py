...
from pay_api.services.fas.routing_slip import RoutingSlip as RoutingSlipService
...
def test_routing_slip_find_search(session):
    ...
    res = RoutingSlipService.search(RoutingSlipSearch(search_filter=search_dict, page=1, limit=1, return_all=True))
    assert res
    assert len(res["items"]) == 1, "searched with routing slip.so only one record"

    res = RoutingSlipService.search(RoutingSlipSearch(search_filter={}, page=1, limit=1, return_all=True))
    assert res
    assert len(res["items"]) == 21, "retun all true ;so shud return all records"

    res = RoutingSlipService.search(RoutingSlipSearch(search_filter={}, page=1, limit=1, return_all=False))
    assert res
    assert len(res["items"]) == 1, "return all false"
