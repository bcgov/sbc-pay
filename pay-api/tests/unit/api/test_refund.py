...
def test_create_refund(session, client, jwt, app, send_email_mock, account_admin_mock, monkeypatch):
...
def test_create_eft_refund(session, client, jwt, app):
    ...
    rv = client.post(
        f"/api/v1/payment-requests/{inv_id2}/refunds",
        data=json.dumps({"reason": "Test"}),
        headers=headers,
    )
