...
def test_calculate_fees_zero_filing_fee_without_service_fee(session, client, jwt, app):
    """Assert that service fee is zero when the schedule has no service fee configured."""
    token = jwt.create_jwt(get_claims(), token_header)
    headers = {"Authorization": f"Bearer {token}", "content-type": "application/json"}
...
def test_calculate_fees_zero_filing_fee_with_service_fee(session, client, jwt, app):
    """Assert that the service fee still applies when the filing fee is zero."""
    token = jwt.create_jwt(get_claims(), token_header)
    headers = {"Authorization": f"Bearer {token}", "content-type": "application/json"}
