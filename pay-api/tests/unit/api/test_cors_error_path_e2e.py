...
REGEX_ORIGIN = r"^https://dev\.?.*\.bcregistry\.gov\.bc\.ca$"
...
@pytest.fixture
def regex_cors_origins_override(app):
    """Scope a regex CORS_ORIGINS override to this file, restoring it after each test."""
    original = app.config.get("CORS_ORIGINS")
    app.config["CORS_ORIGINS"] = [REGEX_ORIGIN]
    yield
    app.config["CORS_ORIGINS"] = original
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "https://dev.create.business.bcregistry.gov.bc.ca",
    ],
)
def test_regex_cors_origin_entry_matches_on_401(app, client, jwt, session, regex_cors_origins_override, origin):
    """Assert CORS_ORIGINS regex properly fails with 401s."""
    rv = client.get("/api/v1/fees/BC/BCINC", headers={"Origin": origin})
    assert rv.status_code == 401
    assert rv.headers.get("Access-Control-Allow-Origin") == origin


def test_regex_cors_origin_entry_rejects_non_matching_origin_on_401(
    app, client, jwt, session, regex_cors_origins_override
):
    """Assert CORS_ORIGINS regex still rejects an origin outside its pattern on the error path."""
    rv = client.get("/api/v1/fees/BC/BCINC", headers={"Origin": "https://other.com"})
    assert rv.status_code == 401
    assert "Access-Control-Allow-Origin" not in rv.headers
