...
def test_link_routing_slip_parent_is_a_child(session, client, jwt):
    ...
    token = jwt.create_jwt(
        get_claims(roles=[Role.FAS_CREATE.value, Role.FAS_LINK.value, Role.FAS_SEARCH.value]),
        token_header,
    )
    headers = {"Authorization": f"Bearer {token}", "content-type": "application/json"}
...
def test_link_routing_slip(session, client, jwt, app):
    ...
    assert rv.json.get("parent").get("remainingAmount") == paid_amount_child + paid_amount_parent
...
def test_update_routing_slip_refund_status(session, client, jwt, app):
    ...
    rv = client.patch(
        ...
        data=json.dumps({"refund_status": ChequeRefundStatus.CHEQUE_UNDELIVERABLE.value}),
        headers=headers,
    )
    assert rv.status_code == 200
    assert rv.json.get("refundStatus") == ChequeRefundStatus.CHEQUE_UNDELIVERABLE.value
...
def test_routing_slip_void(session, client, jwt, app):
    ...
    token = jwt.create_jwt(get_claims(roles=[Role.FAS_EDIT.value, Role.FAS_VOID.value]), token_header)
    headers = {"Authorization": f"Bearer {token}", "content-type": "application/json"}
...
def test_routing_slip_correction(session, client, jwt, app):
    ...
    token = jwt.create_jwt(
        get_claims(roles=[Role.FAS_VIEW.value, Role.FAS_EDIT.value, Role.FAS_CORRECTION.value]),
        token_header,
    )
    headers = {"Authorization": f"Bearer {token}", "content-type": "application/json"}
