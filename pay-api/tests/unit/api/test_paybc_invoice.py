...
def test_paybc_invoice_for_credit_card(client, jwt, app):
    ...
    rv = client.post('/api/v1/paybc/invoices', data=json.dumps(INVOICE_REQUEST_CC), headers=headers)
    mock_responses.stop()
...
def test_paybc_invoice_for_non_credit_card(client, jwt, app):
    ...
    rv = client.post('/api/v1/paybc/invoices', data=json.dumps(INVOICE_REQUEST_NON_CC), headers=headers)
    mock_responses.stop()
