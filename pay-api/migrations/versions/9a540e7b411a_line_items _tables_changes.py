...
def upgrade():
    op.add_column('payment', sa.Column('paid', sa.Float(), autoincrement=False, nullable=True))

    op.add_column('payment_line_item', sa.Column('line_item_status_code', sa.String(length=10), nullable=True))
    ...
    op.create_foreign_key(
        'fk_line_item_status', 'payment_line_item', 'status_code', ['line_item_status_code'], ['code']
    )

    status_code_table = table('status_code', column('code', String), column('description', String))
