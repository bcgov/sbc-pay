...
def upgrade():
    ...
    corp_type_table = table(
        "corp_types", column("code", String), column("description", String)
    )
    ...
    op.bulk_insert(corp_type_table, [{"code": "BCA", "description": "BC Assessment"}])
...
def downgrade():
    ...
    op.execute(
        "delete from fee_schedules where filing_type_code in ('OLAARTOQ','OLAARTAQ','OLAARTIQ') "
    )
    op.execute(
        "delete from filing_types where code in ('OLAARTOQ','OLAARTAQ','OLAARTIQ') "
    )
    op.execute("delete from corp_types where code in ('BCA') ")
    op.execute(
        "delete from distribution_codes where name = 'BC Assessment' and created_by = 'Alembic'"
    )
    # ### end Alembic commands ###
