...
def upgrade():
    with op.batch_alter_table('cfs_accounts', schema=None) as batch_op:
        batch_op.add_column(sa.Column('payment_method', sa.String(length=15), nullable=True))
        batch_op.create_foreign_key(None, 'payment_methods', ['payment_method'], ['code'])

    with op.batch_alter_table('cfs_accounts_history', schema=None) as batch_op:
        batch_op.add_column(sa.Column('payment_method', sa.String(length=15), autoincrement=False, nullable=True))
        batch_op.create_foreign_key(None, 'payment_methods', ['payment_method'], ['code'])
