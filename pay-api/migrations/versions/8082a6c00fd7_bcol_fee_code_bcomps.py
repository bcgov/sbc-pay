...
def upgrade():
    ...
    op.execute("update payment_status_code set description='Completed' where code = 'COMPLETED' ")


def downgrade():
    op.execute("update corp_type set bcol_fee_code='' where code in ('BC')")
    op.execute("update payment_status_code set description='Paid' where code = 'COMPLETED' ")
