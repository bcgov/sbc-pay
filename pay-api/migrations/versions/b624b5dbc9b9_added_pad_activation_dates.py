...
def upgrade():
    ...
    op.alter_column('cfs_account_status_code', 'code',
                    existing_type=sa.VARCHAR(length=20),
                    type_=sa.String(length=40))

    cfs_account_status_code_table = table('cfs_account_status_code',
                                          column('code', String),
    ...
    op.bulk_insert(
        ...
        [
            {'code': 'PENDING_PAD_ACTIVATION', 'description': 'Account in PAD Confirmation Period.'},
        ]
    )
...
def downgrade():
    ...
    op.alter_column('cfs_account', 'status',
                    ...
                    type_=sa.VARCHAR(length=20),
                    existing_nullable=True)
    op.execute("delete from cfs_account_status_code where code='PENDING_PAD_ACTIVATION'")
