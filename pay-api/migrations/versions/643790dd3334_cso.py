...
def upgrade():
    ...
    op.bulk_insert(
        filing_type_table, [{"code": "CSBFILE", "description": "CSB Filing Charge"}]
    )
    ...
    if (res_fetch := res.fetchall()) and res_fetch[0]:
        ...
        res = conn.execute(
            sa.text(
                f"select fee_schedule_id from fee_schedules where corp_type_code='CSO' and filing_type_code='CSBFILE'"
            )
        )
