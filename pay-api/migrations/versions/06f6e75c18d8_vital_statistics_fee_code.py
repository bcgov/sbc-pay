...
def upgrade():
    ...
    op.bulk_insert(
        corp_type_table,
        [
            {'code': 'VS', 'description': 'Vital Statistics', 'transaction_fee_code': 'TRF01'}
        ]
    )
...
def downgrade():
    op.execute("delete from fee_schedule where filing_type_code in ('WILLNOTICE','WILLSEARCH','WILLALIAS') ")
    op.execute("delete from filing_type where code in ('WILLNOTICE','WILLSEARCH','WILLALIAS') ")
