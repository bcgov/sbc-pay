...
def upgrade():
    ...
    op.bulk_insert(
        corp_type_table,
        [
            {'code': 'ULC', 'description': 'Unlimited Liability Company', 'bcol_fee_code': 'BBCOMVC1',
    ...
    if len(ben_code) == 0:
        ...
        op.bulk_insert(
            corp_type_table,
            [
                {'code': 'BEN', 'description': 'Benefit Company', 'bcol_fee_code': 'BBCOMVC1',
                 'bcol_staff_fee_code': 'CBCOMVC1'}
            ]
        )
        # Update all the fee_schedule with BEN
        op.execute("update fee_schedule set corp_type_code = 'BEN' where corp_type_code = 'BC'")
    ...
    op.bulk_insert(
        filing_type_table,
        [
            {'code': 'TRANS', 'description': 'Transition Application'}
...
def downgrade():
    # ### commands auto generated by Alembic - please adjust! ###
    op.execute("delete from fee_schedule where filing_type_code='TRANS' and corp_type_code in ('BC', 'BEN')")
    op.execute("delete from fee_schedule where filing_type_code='ALTER' and corp_type_code='ULC'")
