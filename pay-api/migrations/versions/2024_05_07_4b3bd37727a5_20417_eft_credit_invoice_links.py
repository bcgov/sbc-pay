...
def upgrade():
    with op.batch_alter_table('eft_credit_invoice_links', schema=None) as batch_op:
        batch_op.add_column(sa.Column('amount', sa.Numeric(precision=19, scale=2), nullable=True))
        batch_op.add_column(sa.Column('status_code', sa.String(length=25), nullable=True))
        batch_op.create_index(batch_op.f('ix_eft_credit_invoice_links_status_code'), ['status_code'], unique=False)

    op.execute(text(f"UPDATE eft_credit_invoice_links "
                    f"SET status_code = 'PENDING'"))
