...
def upgrade():
    ...
    res = conn.execute(
        text(
            f'select i.id as "id", p.payment_date as "date"\
    ...
    for payment in payments:
        ...
        op.execute(
            text(
                f"update invoices set payment_date='{payment_date}' where id = {invoice_id}"
            )
        )
    ...
    res = conn.execute(
        text(
            f'select i.id as "id", r.requested_date as "date"\
                                    from invoices i \
    ...
    for refund in refunds:
        ...
        op.execute(
            text(f"update invoices set refund_date='{refund_date}' where id = {id}")
        )
