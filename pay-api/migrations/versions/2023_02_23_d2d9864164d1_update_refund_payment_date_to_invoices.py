...
def upgrade():
    ...
    for payment in payments:
        ...
        op.execute(f"update invoices set payment_date=\'{payment_date}\' where id = {invoice_id}")

    # 1. Find all id and refund date from invoice table linking with refunds table.
    # 2. Update refund_date with invoice_id.
    res = conn.execute(f'select i.id as "id", r.requested_date as "date"\
                                    from invoices i \
    ...
    for refund in refunds:
        ...
        op.execute(f"update invoices set refund_date=\'{refund_date}\' where id = {id}")
