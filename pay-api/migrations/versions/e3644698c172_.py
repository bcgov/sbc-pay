...
def upgrade():
    ...
    op.alter_column('account_fees', 'created_on',
...
def downgrade():
    ...
    op.create_index('routing_slips_status_idx', 'routing_slips', ['status'], unique=False)
    op.drop_index(op.f('ix_invoices_payment_method_code'), table_name='invoices')
    op.create_index('invoices_payment_method_code_idx', 'invoices', ['payment_method_code'], unique=False)
    op.alter_column('account_fees', 'created_by',
               existing_type=sa.VARCHAR(length=50),
               nullable=True)
    op.alter_column('account_fees', 'created_on',
