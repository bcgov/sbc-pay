...
def upgrade():
    ...
    rs_status_table = op.create_table('routing_slip_status_codes',
    ...
    op.bulk_insert(
        ...
        [
            ...
            {'code': 'NSF', 'description': 'No Sufficient Fund'},
            {'code': 'REFUND', 'description': 'Refund'},
            {'code': 'LAST', 'description': 'Last Service'}
        ]
    )

    op.create_table('routing_slips',
    ...
    op.alter_column('payment_accounts_version', 'auth_account_name', new_column_name='name')

    op.add_column('payments', sa.Column('cheque_receipt_number', sa.String(length=50), nullable=True,
                                        comment='Cheque or cash receipt number'))
    op.add_column('payments', sa.Column('routing_slip_number', sa.String(length=50), nullable=True,
    ...
    op.alter_column('payments', 'receipt_number',
                    ...
                    existing_nullable=True)
    op.alter_column('payments', 'paid_amount',
    ...
    op.alter_column('payments', 'created_by',
                    ...
                    existing_nullable=True)
    op.create_index(op.f('ix_payments_routing_slip_number'), 'payments', ['routing_slip_number'], unique=False)
    op.create_index(op.f('ix_payments_cheque_receipt_number'), 'payments', ['cheque_receipt_number'],
                    unique=False)
...
def downgrade():
    ...
    op.drop_index(op.f('ix_payments_routing_slip_number'), table_name='payments')
    op.alter_column('payments', 'created_by',
                    existing_type=sa.VARCHAR(length=50),
                    comment=None,
    ...
    op.drop_column('payments', 'routing_slip_number')
    op.drop_column('payments', 'cheque_receipt_number')
    op.alter_column('payment_accounts', 'name', new_column_name='auth_account_name')
    op.alter_column('payment_accounts_version', 'name', new_column_name='auth_account_name')
