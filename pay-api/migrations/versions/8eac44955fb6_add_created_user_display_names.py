...
def upgrade():
    ...
    op.add_column('invoice', sa.Column('created_name', sa.String(length=100), nullable=True))
    op.add_column('invoice', sa.Column('updated_name', sa.String(length=100), nullable=True))
    op.add_column('payment', sa.Column('created_name', sa.String(length=100), nullable=True))
    op.add_column('payment', sa.Column('updated_name', sa.String(length=100), nullable=True))
    ...
    if len(pay_results) > 0 or len(inv_results) > 0:
        ...
        token_response = OAuthService.post(token_url, basic_auth_encoded, AuthHeaderType.BASIC, ContentType.FORM_URL_ENCODED, data)
        token = token_response.json().get('access_token')

        get_users_url = issuer_url.replace('/auth/', '/auth/admin/') + '/users?max=999999999'
        get_users_response = OAuthService.get(get_users_url, token, AuthHeaderType.BEARER, ContentType.JSON)
        for user in get_users_response.json():
            if user.get('firstName', None) or user.get('lastName', None):
                user_name = user['username'].lower()
                display_name = user.get('firstName', '') + ' ' + user.get('lastName', '')
                op.execute(f"update payment set created_name = '{display_name}'  where lower(created_by) = '{user_name}'")
                op.execute(f"update payment set updated_name = '{display_name}'  where lower(updated_by) = '{user_name}'")
                op.execute(f"update invoice set created_name = '{display_name}'  where lower(created_by) = '{user_name}'")
                op.execute(f"update invoice set updated_name = '{display_name}'  where lower(updated_by) = '{user_name}'")
