...
def upgrade():
    ...
    disbursement_status_codes = op.create_table(
    ...
    op.create_index(
        op.f("ix_ejv_files_file_ref"), "ejv_files", ["file_ref"], unique=False
    )
    op.create_table(
        "ejv_invoice_links",
        sa.Column("id", sa.Integer(), autoincrement=True, nullable=False),
    ...
    op.alter_column(
        "distribution_codes", "memo_name", nullable=True, new_column_name="name"
    )
    # op.add_column('distribution_codes', sa.Column('name', sa.String(length=50), nullable=True))
    op.add_column(
        "distribution_codes",
        sa.Column("disbursement_distribution_code_id", sa.Integer(), nullable=True),
    )
    op.add_column(
        "distribution_codes",
        sa.Column("service_fee_distribution_code_id", sa.Integer(), nullable=True),
    )
    op.create_foreign_key(
        "service_fee_distribution_code_id_fk",
        "distribution_codes",
        "distribution_codes",
    ...
    op.create_foreign_key(
        "disbursement_distribution_code_id_fk",
        "distribution_codes",
        "distribution_codes",
    ...
    for result in results:
        ...
        res = conn.execute(
            sa.text(
                f"select distribution_code_id from distribution_codes "
    ...
    op.execute(
        "update distribution_codes set service_fee_distribution_code_id=null where name='NSF';"
    )
...
def downgrade():
    ...
    op.add_column(
        "distribution_codes",
        sa.Column(
            "service_fee_responsibility_centre",
    ...
    op.drop_constraint(
        "service_fee_distribution_code_id_fk", "distribution_codes", type_="foreignkey"
    )
    op.drop_constraint(
        "disbursement_distribution_code_id_fk", "distribution_codes", type_="foreignkey"
    )
    op.drop_column("distribution_codes", "service_fee_distribution_code_id")
    # op.drop_column('distribution_codes', 'name')
    op.alter_column(
        "distribution_codes", "name", nullable=True, new_column_name="memo_name"
    )
