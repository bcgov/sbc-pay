...
def upgrade():
    ...
    with op.batch_alter_table("payment_accounts", schema=None) as batch_op:
        ...
        batch_op.add_column(
            sa.Column("has_overdue_invoices", sa.DateTime(), nullable=True)
        )

    with op.batch_alter_table("payment_accounts_history", schema=None) as batch_op:
        batch_op.add_column(
            sa.Column(
                "has_nsf_invoices", sa.DateTime(), autoincrement=False, nullable=True
            )
        )
        batch_op.add_column(
            sa.Column(
    ...
    op.execute(
        "update payment_accounts pa set has_nsf_invoices = (select now() from cfs_accounts ca where ca.account_id = pa.id and ca.status = 'FREEZE' and ca.payment_method = 'PAD' limit 1)"
    )
    op.execute(
        "update payment_accounts pa set has_overdue_invoices = (select now() from invoices i where i.payment_method_code = 'EFT' and i.payment_account_id = pa.id and i.invoice_status_code = 'OVERDUE' limit 1)"
    )
