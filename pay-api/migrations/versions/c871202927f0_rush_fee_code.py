...
def upgrade():
    ...
    op.bulk_insert(fee_code_table, [{"code": "EN204", "amount": 33}])
    op.bulk_insert(filing_type_table, [{"code": "WILLRUSH", "description": "Rush Fee"}])
    ...
    op.execute(
        "update fee_schedules set priority_fee_code=null where corp_type_code='VS'"
    )
...
def downgrade():
    ...
    op.execute(
        "DELETE FROM fee_schedules WHERE filing_type_code='WILLRUSH' and corp_type_code='VS'"
    )
    op.execute("DELETE FROM filing_types WHERE code='WILLRUSH'")

    op.execute("DELETE FROM fee_codes WHERE code = 'EN204'")
    op.execute(
        "update fee_schedules set priority_fee_code='PRI02' where corp_type_code='VS'"
    )
