...
def upgrade():
    ...
    op.execute(
        "update filing_types set description='PPR Registration (year)' where code='FSREG'"
    )
    op.execute(
        "update filing_types set description='PPR Registration (infinity)' where code='INFRG'"
    )
    ...
    # Fee Codes
    op.bulk_insert(fee_code_table, [{"code": "EN114", "amount": 10}])
    ...
    op.bulk_insert(
        ...
        [
            {
                "code": "FLREG",
                "description": "PPR Registration under the Family Law Act",
            },
...
def downgrade():
    # ### commands auto generated by Alembic - please adjust! ###
    op.execute(
        "delete from fee_schedules where filing_type_code in ('FSCHG', 'INFRN', 'FSREN', 'RLREG', 'FLREG')"
    )
    op.execute(
        "delete from filing_types where code in ('FSCHG', 'INFRN', 'FSREN', 'RLREG', 'FLREG')"
    )
