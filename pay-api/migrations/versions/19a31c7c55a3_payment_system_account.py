...
Revision ID: 19a31c7c55a3
Revises: 5cdd1c1d355e
Create Date: 2020-02-26 12:44:09.477882
...
def upgrade():
    ...
    payment_accounts: [] = []
    conn = op.get_bind()
    res = conn.execute(f"select id,corp_number,corp_type_code,payment_system_code,account_number,party_number,site_number from payment_account;")
    ...
    for payment_account in payment_accounts:
        ...
        if corp_number:
            ...
            if internal_payment_account_id:
                op.execute(
                    f"update invoice set internal_account_id = '{internal_payment_account_id}' where account_id = '{id}'")
            elif credit_payment_account_id:
                op.execute(
                    f"update invoice set credit_account_id = '{credit_payment_account_id}' where account_id = '{id}'")
