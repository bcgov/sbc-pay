...
def upgrade():
    ...
    op.execute(
        sa.schema.CreateSequence(
            sa.Sequence("eft_group_link_seq", data_type=sa.Integer)
        )
    )
    ...
    with op.batch_alter_table("eft_short_names_historical", schema=None) as batch_op:
        batch_op.create_index(
            batch_op.f("ix_eft_short_names_historical_hidden"), ["hidden"], unique=False
        )
        batch_op.create_index(
            batch_op.f("ix_eft_short_names_historical_payment_account_id"),
            ["payment_account_id"],
            unique=False,
        )
        batch_op.create_index(
            batch_op.f("ix_eft_short_names_historical_related_group_link_id"),
            ["related_group_link_id"],
            unique=False,
        )
        batch_op.create_index(
            batch_op.f("ix_eft_short_names_historical_transaction_date"),
            ["transaction_date"],
            unique=False,
        )
    ...
    with op.batch_alter_table("eft_credits", schema=None) as batch_op:
        ...
        batch_op.drop_constraint(
            "eft_credits_payment_account_id_fkey", type_="foreignkey"
        )
...
def downgrade():
    ...
    with op.batch_alter_table("eft_credits", schema=None) as batch_op:
        ...
        batch_op.create_foreign_key(
            "eft_credits_payment_account_id_fkey",
            "payment_accounts",
            ["payment_account_id"],
            ["id"],
        )
        batch_op.create_index(
            "ix_eft_credits_payment_account_id", ["payment_account_id"], unique=False
        )
