...
def send_email(note_book, emailtype, errormessage):
    ...
    if emailtype == 'ERROR':
        subject = "Jupyter Notebook Error Notification from sbc-pay for processing '" \
        ...
        if file_processing == 'pay':
            subject = 'SBC-PAY Weekly Stats till ' + datetime.strftime(datetime.now()-timedelta(1), '%Y-%m-%d') + ext
            filename = 'pay_weekly_stats_till_' + datetime.strftime(datetime.now()-timedelta(1), '%Y-%m-%d') + '.csv'
            recipients = os.getenv('PAY_WEEKLY_REPORT_RECIPIENTS', '')
...
def processnotebooks(notebookdirectory):    
    ...
    try:
        ...
        if notebookdirectory == 'weekly':
            week_report_dates = ast.literal_eval(os.getenv('PAY_WEEKLY_REPORT_DATES', ''))
    except Exception:
