...
from .factory import (
    factory_create_direct_pay_account, factory_create_pad_account, factory_invoice, factory_invoice_reference,
    factory_payment)


@pytest.mark.parametrize('test_name, payment_method, invoice_total, refund_total, start_status, expected, mismatch', [
    ('drawdown_refund_full', PaymentMethod.DRAWDOWN.value, 31.5, -31.5,
     InvoiceStatus.REFUND_REQUESTED.value, InvoiceStatus.REFUNDED.value, False),
    ('drawdown_refund_partial', PaymentMethod.DRAWDOWN.value, 31.5, -10,
...
def test_bcol_refund_confirmation(
        session, monkeypatch, test_name, payment_method, invoice_total, refund_total, start_status, expected, mismatch):
