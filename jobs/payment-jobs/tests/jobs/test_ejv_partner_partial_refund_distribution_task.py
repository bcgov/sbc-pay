...
from pay_api.models import CorpType as CorpTypeModel, DistributionCode, EjvFile, EjvHeader, EjvInvoiceLink, FeeSchedule, Invoice, db
...
from .factory import (factory_create_direct_pay_account, factory_distribution, factory_distribution_link, factory_invoice, factory_invoice_reference, factory_payment, factory_payment_line_item, factory_receipt)
...
def test_partial_refund_disbursement_for_partners(session, monkeypatch, client_code, batch_type):
    ...
    invoice = factory_invoice(payment_account=pay_account, corp_type_code=corp_type.code, total=21.5, status_code='PAID')
    factory_payment_line_item(invoice_id=invoice.id, fee_schedule_id=fee_schedule.fee_schedule_id, filing_fees=10, total=10, service_fees=1.5, fee_dist_id=fee_distribution.distribution_code_id)
    
    # Partially refunded line item
    pli_refunded = factory_payment_line_item(invoice_id=invoice.id, fee_schedule_id=fee_schedule.fee_schedule_id, filing_fees=10, total=10, service_fees=1.5, fee_dist_id=fee_distribution.distribution_code_id)
